...
public class FacialRecognition {
	...
	private static class Training {
		...
		Training(FaceDb db) {
			...
			for(String name : namesInDb) {
				for (BufferedImage image : db.get(name)) {
					...
					////          CvArr. arr[] = ImageIO.write(image, "jpg", output);
					////      images.put(imgCount, ImageIO.write(image, "jpg", output));
		...
		// /   public boolean toDir (BufferedImage b , )
		...
		void identify(BufferedImage image, PotentialFace face) {
			...
			if (prediction[0] >= 0 && prediction[0] < names.length) {
				...
				if (face.confidence>=100){
					noFunc();
					//        	System.out.println("unkown");
					//    	    pb = new ProcessBuilder("espeak","unknown person");
					...
					//        	
					//	    	pb = new ProcessBuilder("aplay","/home/abuamra/workspace/OCVV/soundNames/"+face.name+".wav");
					//	    	pb.redirectErrorStream(true);
					//	    	
					//			try {
					//				p = pb.start();
					...
					//    	    pb = new ProcessBuilder("espeak","\""+face.name+"\"");
					//    	    pb.redirectErrorStream(true);
					//    	    try {
					//				p = pb.start();
					//			} catch (IOException e) {
		...
		void noFunc(){
			noCounter++;
			System.out.println("noFunc:"+ noCounter);
			yesCounter=0;
			if (noCounter==7){
				System.out.println("unkown");
				pb = new ProcessBuilder("espeak","unknown person");
		...
		void yesFunc(){
			noCounter=0;
			System.out.println("yesFunc");
			if (conFaceName==null||faceName==conFaceName){
				yesCounter++;
				System.out.println("yesFunc:"+ yesCounter);
				if (yesCounter==7){
					System.out.println(faceName);

					pb = new ProcessBuilder("aplay","/root/workspace/FaceRecognition/soundNames/"+faceName+".wav");
					pb.redirectErrorStream(true);
